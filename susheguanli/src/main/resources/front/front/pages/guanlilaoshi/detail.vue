	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.guanlilaoshiId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('guanlilaoshi', this.guanlilaoshiId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.guanlilaoshiPhoto ? this.detail.guanlilaoshiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('guanlilaoshiYuyue', {
									xueshengId: _this.user.id,
									guanlilaoshiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/guanlilaoshiYuyue/list');
							}
						}
					});
				},
