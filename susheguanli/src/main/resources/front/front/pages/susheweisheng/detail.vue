	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.susheweishengId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('susheweisheng', this.susheweishengId);
                    this.detail = res.data
					this.detail.susheweishengContent = this.detail.susheweishengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.susheweishengPhoto ? this.detail.susheweishengPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('susheweishengYuyue', {
									xueshengId: _this.user.id,
									susheweishengId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/susheweishengYuyue/list');
							}
						}
					});
				},
