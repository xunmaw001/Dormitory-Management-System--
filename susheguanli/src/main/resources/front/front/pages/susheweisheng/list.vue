	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"var(--publicSubColor)","color":"#fff","textAlign":"left","borderRadius":"20rpx 0","borderWidth":"0 10rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.susheName" type="text" placeholder="宿舍名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('susheweisheng','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('susheweisheng','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='susheweishengTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in susheweishengTypesList" :key="index" :class="susheweishengTypes===item.codeIndex?'tabActive':''"
                                  @tap="susheweishengTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.susheweishengName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('susheweisheng','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.susheName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">宿舍名称</text>
                    <text style="width: 70%">{{item.susheName}}</text>
                </view>
                <!-- 当前表 -->
                 <view v-show="item.susheweishengUuidNumnber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">宿舍卫生编号</text>
                     <text style="width: 70%">
                         <text v-if="item.susheweishengUuidNumnber != null">{{item.susheweishengUuidNumnber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.susheweishengValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">卫生状态</text>
                     <text style="width: 70%">{{item.susheweishengValue}}</text>
                 </view>
                 <view v-show="item.dafen" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">打分</text>
                     <text style="width: 70%">
                         <text v-if="item.dafen != null">{{item.dafen}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.riqiTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">所属日期</text>
                    <text style="width: 70%">
                        <text v-if="item.riqiTime != null">{{item.riqiTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                <view v-show="item.susheweishengContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">卫生详情</text>
                    <view style="width: 70%" v-html="item.susheweishengContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('susheweisheng','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('susheweisheng','审核') && item.susheweishengYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 卫生状态 字段 字典表查询方法
            async susheweishengTypesSelect(){
                let susheweishengTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'susheweisheng_types',
                });
                susheweishengTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.susheweishengTypesList = susheweishengTypes.data.list
            },
            //当前表的 卫生状态字段类别搜索
			susheweishengTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.susheweishengTypes != 0) {
                    params.susheweishengTypes = this.susheweishengTypes
                }
                let _this = this
                let res = await this.$api.page(`susheweisheng`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('susheweisheng', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.susheweishengShenheTime=shijian;
                            await _this.$api.update(`susheweisheng`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.susheweishengUuidNumnber) {
                    searchForm['susheweishengUuidNumnber'] = this.searchForm.susheweishengUuidNumnber
                }
				if (this.searchForm.susheName) {
                    searchForm['susheName'] = this.searchForm.susheName
                }

                let res = await this.$api.page(`susheweisheng`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.susheweishengContent!=null){
                        item.susheweishengContent = item.susheweishengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
