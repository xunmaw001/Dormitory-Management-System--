	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('sushe', this.susheId);
                    this.detail = res.data
					this.detail.susheContent = this.detail.susheContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.sushePhoto ? this.detail.sushePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('susheYuyue', {
									xueshengId: _this.user.id,
									susheId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/susheYuyue/list');
							}
						}
					});
				},
