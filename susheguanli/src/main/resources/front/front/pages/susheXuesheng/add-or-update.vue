    ...
    <view class="content">
        <form class="app-update-pv">
                ...
                <!--<view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                    ...
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">宿舍</view>
                    <picker @change="susheChange" :value="susheIndex" :range="susheOptions" range-key="susheName">
                        <view
                                :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.susheId?ruleForm.susheName:"请选择宿舍"}}</view>
                    </picker>
                </view>-->
...
<script>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            let susheData = await this.$api.page(`sushe`, susheParams);
            this.susheOptions = susheData.data.list;
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`susheXuesheng`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.susheXueshengId){
                this.ruleForm.susheXueshengId = options.susheXueshengId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
                if (this.ruleForm.id) {
                    await this.$api.update(`susheXuesheng`, this.ruleForm);
                } else {
                    await this.$api.save(`susheXuesheng`, this.ruleForm);
                }
