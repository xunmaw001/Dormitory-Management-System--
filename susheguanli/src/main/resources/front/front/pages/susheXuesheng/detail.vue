	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.susheXueshengId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('susheXuesheng', this.susheXueshengId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.susheXueshengPhoto ? this.detail.susheXueshengPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('susheXueshengYuyue', {
									xueshengId: _this.user.id,
									susheXueshengId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/susheXueshengYuyue/list');
							}
						}
					});
				},
