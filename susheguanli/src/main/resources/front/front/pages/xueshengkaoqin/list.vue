	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	 ...
	 <!--<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="action">
            ...
            <view v-if="isAuth('xueshengkaoqin','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('xueshengkaoqin','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xueshengkaoqinTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xueshengkaoqinTypesList" :key="index" :class="xueshengkaoqinTypes===item.codeIndex?'tabActive':''"
                                  @tap="xueshengkaoqinTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.xueshengkaoqinName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('xueshengkaoqin','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.xueshengkaoqinValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">考勤结果</text>
                     <text style="width: 70%">{{item.xueshengkaoqinValue}}</text>
                 </view>
                <view v-show="item.xueshengkaoqinContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">考勤详情</text>
                    <view style="width: 70%" v-html="item.xueshengkaoqinContent"></view>
                </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('xueshengkaoqin','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('xueshengkaoqin','审核') && item.xueshengkaoqinYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.xueshengkaoqinTypesSelect()
		...
		methods: {
            ...
            //当前表的 考勤结果 字段 字典表查询方法
            async xueshengkaoqinTypesSelect(){
                let xueshengkaoqinTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xueshengkaoqin_types',
                });
                xueshengkaoqinTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xueshengkaoqinTypesList = xueshengkaoqinTypes.data.list
            },
            //当前表的 考勤结果字段类别搜索
			xueshengkaoqinTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xueshengkaoqinTypes != 0) {
                    params.xueshengkaoqinTypes = this.xueshengkaoqinTypes
                }
                let _this = this
                let res = await this.$api.page(`xueshengkaoqin`, params);
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.xueshengkaoqinShenheTime=shijian;
                            await _this.$api.update(`xueshengkaoqin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`xueshengkaoqin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xueshengkaoqinContent!=null){
                        item.xueshengkaoqinContent = item.xueshengkaoqinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
