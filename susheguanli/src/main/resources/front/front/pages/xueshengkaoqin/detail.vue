	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xueshengkaoqinId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xueshengkaoqin', this.xueshengkaoqinId);
                    this.detail = res.data
					this.detail.xueshengkaoqinContent = this.detail.xueshengkaoqinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xueshengkaoqinPhoto ? this.detail.xueshengkaoqinPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xueshengkaoqinYuyue', {
									xueshengId: _this.user.id,
									xueshengkaoqinId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xueshengkaoqinYuyue/list');
							}
						}
					});
				},
