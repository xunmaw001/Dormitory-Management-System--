        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '管理老师姓名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.guanlilaoshiName" placeholder="管理老师姓名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('guanlilaoshi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/susheguanli/upload/guanlilaoshiMuBan.xls"
                        >批量导入管理老师数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('guanlilaoshi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="guanlilaoshiUploadSuccess"
                                :on-error="guanlilaoshiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('guanlilaoshi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入管理老师数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('guanlilaoshi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "guanlilaoshi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanlilaoshiName"
                                   header-align="center"
                                   label="管理老师姓名">
                        <template slot-scope="scope">
                            {{scope.row.guanlilaoshiName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanlilaoshiPhone"
                                   header-align="center"
                                   label="管理老师手机号">
                        <template slot-scope="scope">
                            {{scope.row.guanlilaoshiPhone}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanlilaoshiIdNumber"
                                   header-align="center"
                                   label="管理老师身份证号">
                        <template slot-scope="scope">
                            {{scope.row.guanlilaoshiIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="guanlilaoshiPhoto"
                                     header-align="center"
                                     width="200"
                                     label="管理老师头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.guanlilaoshiPhoto">
                                <img :src="$base.url+scope.row.guanlilaoshiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanlilaoshiEmail"
                                   header-align="center"
                                   label="管理老师邮箱">
                        <template slot-scope="scope">
                            {{scope.row.guanlilaoshiEmail}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jinyongTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('guanlilaoshi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('guanlilaoshi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('guanlilaoshi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('guanlilaoshi','修改')" type="success" size="mini" @click="jinqiyong(scope.row.id,scope.row.jinyongTypes)">{{scope.row.jinyongTypes == 1?'禁用账户':'启用账户'}}</el-button>

                            <el-button v-if="isAuth('guanlilaoshi','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                username : null,
                password : null,
                guanlilaoshiName : null,
                guanlilaoshiPhone : null,
                guanlilaoshiIdNumber : null,
                guanlilaoshiPhoto : null,
                sexTypes : null,
                guanlilaoshiEmail : null,
                jinyongTypes : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '账户': "username",
                     '管理老师姓名': "guanlilaoshiName",
                     '管理老师手机号': "guanlilaoshiPhone",
                     '管理老师身份证号': "guanlilaoshiIdNumber",
                     '管理老师头像': "guanlilaoshiPhoto",
                     '性别': "sexValue",
                     '管理老师邮箱': "guanlilaoshiEmail",
                     '账户状态': "jinyongValue",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'guanlilaoshi',//当前表表名,
                        sumColum : 'guanlilaoshi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'guanlilaoshi_name',//分组字符串字段
                        // types : 'guanlilaoshi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.guanlilaoshiName!= '' && this.searchForm.guanlilaoshiName!= undefined) {
                    params['guanlilaoshiName'] = '%' + this.searchForm.guanlilaoshiName + '%'
                }
                ...
                params['guanlilaoshiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "guanlilaoshi/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "guanlilaoshi/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            guanlilaoshiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "guanlilaoshi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            jinqiyong(id,jinyongTypes) {
                ...
                _this.$confirm(msg, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"}).then(() => {
                    _this.$http({url: "guanlilaoshi/update",method: "post",data: {id:id,jinyongTypes:jinyongTypes,}}).then(({data}) => {
                        if(data && data.code === 0){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "guanlilaoshi/update",
                        method: "post",
                        data: {
                            id:id,
//                            guanlilaoshiTypes:guanlilaoshiTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
                            _this.$message({message: "操作成功",type: "success",duration: 1500,onClose: () => {
